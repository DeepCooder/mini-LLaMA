...
def precompute_theta_pos_frequences(head_dim:int, seq_len:int, device:str, theta:float=10000.0):
    assert head_dim % 2 == 0, 'Dimension must be division by 2'
...
class Transformer(nn.Module):
    def __init__(self, args:ModelArgs):
        ...
        self.freqs_complex = precompute_theta_pos_frequences(self.args.dim // self.args.n_heads, self.args.max_seq_len * 2, device=self.args.device)
    def forward(self, tokens:torch.Tensor, start_pos:int):
        batch_szie, seq_len = tokens.shape
